...
// Function to read cookies from .env
function getCookiesFromEnv() {
  return process.env.YOUTUBE_COOKIES || ''; // Get cookies from .env, fallback to empty string if not set
}
...
async function getVideo(vid, res_ = '360p', m = false) {
  const yt = await Innertube.create({
    ...
    cookies: getCookiesFromEnv()  // Add cookies from the .env file
  });
